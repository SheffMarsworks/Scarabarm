...
try
    ...
    pubJoint1 = ros2publisher(node,"/joint1/command","std_msgs/Float64");
...
%% 7. Nested helper & callback functions
    ...
    function publishJS(q)
        ...
        q1 = q(1);        % Joint 1 value
        qRest = q(2:end); % ODrive joints
    
        % Create joint state message
        js = ros2message("sensor_msgs/JointState");
        js.name     = [{'joint_1_to_joint_2'}, jNames(2:end)];
        js.position = [q1, qRest];
    
        % Header timestamp is required!
        t = rostime("now");
        ...
        % Send ODrive joints via Float64MultiArray (normalized)
        od = ros2message("std_msgs/Float64MultiArray");
        od.data = qRest / (2*pi);
        send(pubODrive, od);
    
        % Send Joint 1 command to its own topic
        cmd1 = ros2message("std_msgs/Float64");
        cmd1.data = q1;
        send(pubJoint1, cmd1);
    end
...
%% 8. Trajectory helpers
    ...
    function sendTraj
      ...
      msg.joint_names = [{'joint_1_to_joint_2'}, jNames(2:end)];
